...
const Home = () => {
    ...
    //                   }
    // useEffect(() => {  
    //     axios.post('https://hoblist.com/api/movieList',parameter)
    //     .then(response => {
  ...
  return (
    <>
      ...
      <div className='mt-16'>
        {data.map((item) => {
          return (
          <div className='border-b-2 w-[40%] mx-auto' key={item._id}>
          <div className='flex mx-auto '>
            <div className='mx-5 my-auto'>
              <TbTriangleFilled size={30} className='cursor-pointer mx-auto' />
              <div className='w-full text-center'>
                <p className='py-3'>{item.totalVoted}</p>
              </div>
              <TbTriangleInvertedFilled size={30} className='cursor-pointer mx-auto' />
              <p className='mt-3'>votes</p>
            </div>
            <img src={item.poster} className='w-32 h-48 rounded-lg shadow-lg my-4' alt="" />
            <div className='mx-4 my-auto w-[50%]'>
              <p className='text-2xl'>{item.title}</p>
              <p className='text-gray-700 pt-1 truncate'>Genre: <span className='text-black'>{item.genre}</span></p>
              <p className='text-gray-700 pt-1 truncate'>Director: <span className='text-black'>{item.director}</span></p>
              <p className='text-gray-700 pt-1 truncate'>Starring: <span className='text-black'>{item.stars}</span></p>
              <p className='pt-1'>Mins | {item.language} | {moment(item.releasedDate).format("MMM D")}</p>
              <p className='pt-1 text-blue-500'>{item.pageViews} views | Voted by {item.totalVoted} Person</p>
            </div>
          </div>
          <button className='w-[90%] bg-blue-400 text-white ml-[5%] mb-3 rounded-sm'>Watch Trailer</button>
          </div>
          )
        })}
      </div>
